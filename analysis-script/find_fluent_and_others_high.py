...
"""
Find the pattern:
  Fluency >= fluency_high  AND  (dims_high) >= high_threshold  (all dims)
...
def analyze_method(eval_dir: Path, fluency_high: float, high_threshold: float, dims_high: list[str], top_k: int) -> dict:
    ...
    for xlsx in sorted(eval_dir.glob("*.xlsx")):
        ...
        required = ["Fluency", "n", "unit_idx", *dims_high]
        ...
        others_high = (df[dims_high] >= high_threshold).all(axis=1)
        pattern = fluent & others_high
        ...
        per_model[model] = {
            ...
            "p_others_high_given_fluent": (pattern_rows / fluent_rows) if fluent_rows else 0.0,
    ...
    return {
        ...
        "aggregate": {
            ...
            "p_others_high_given_fluent": (agg_pattern_rows / agg_fluent_rows) if agg_fluent_rows else 0.0,
...
def print_report(method: str, result: dict, fluency_high: float, high_threshold: float, dims_high: list[str], top_k: int):
    print("\n" + "=" * 72)
    print(f"  {method.upper()} — Fluency >= {fluency_high} AND {dims_high} >= {high_threshold}")
    print("=" * 72)

    agg = result["aggregate"]
    print(
        f"\nAGGREGATE: pattern={agg['pattern_rows']}  fluent={agg['fluent_rows']}  total={agg['total_rows']}  "
        f"P(others_high|fluent)={agg['p_others_high_given_fluent']:.3f}  P(pattern)={agg['p_pattern_overall']:.3f}"
    )
    ...
    for model in sorted(result["per_model"].keys()):
        ...
        print(
            ...
            f"P(others_high|fluent)={m['p_others_high_given_fluent']:.3f}"
        )


def main():
    parser = argparse.ArgumentParser(description="Find rows where Fluency and other dimensions are high")
    ...
    parser.add_argument("--high-threshold", type=float, default=4.0, help="High threshold (>=). Default: 4")
    parser.add_argument(
        "--dims-high",
        default=",".join(DEFAULT_DIMS_HIGH),
        help="Comma-separated list of dimensions that must be >= high-threshold",
    )
    ...
    dims_high = [d.strip() for d in str(args.dims_high).split(",") if d.strip()]
    ...
    for method in (["teler", "reasoning"] if args.method == "both" else [args.method]):
        ...
        if not eval_dir.exists():
            ...
            continue
        results[method] = analyze_method(eval_dir, args.fluency_high, args.high_threshold, dims_high, args.top)
        print_report(method, results[method], args.fluency_high, args.high_threshold, dims_high, args.top)
                ...
                for model, m in res["per_model"].items():
                    w.writerow(
                        [
                            ...
                            m["fluent_rows"],
                            m["total_rows"],
                            f"{m['p_others_high_given_fluent']:.6f}",
