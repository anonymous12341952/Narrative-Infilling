#!/usr/bin/env python3
"""
Sample random high-quality evaluation rows (Average >= 4.5) and write to a log file.

Format: dataset, problem, gold_answer, extracted_answer, dimension scores.
Output: high_quality_samples.log
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sample high-quality (avg>=4.5) evaluation rows")
    parser.add_argument("-n", "--num", type=int, default=20, help="Number of samples (default: 20)")
    parser.add_argument("--min-avg", type=float, default=4.5, help="Minimum Average (default: 4.5)")
    parser.add_argument("--method", "-m", choices=["teler", "reasoning", "both"], default="teler")
    parser.add_argument("--output", "-o", default=None, help="Output log path (default: high_quality_samples.log)")
    ...
    parser.add_argument("--max-problem-len", type=int, default=600, help="Only include samples where problem length <= N chars (0=no limit, default: 600)")
    parser.add_argument("--min-problem-len", type=int, default=0, help="Only include samples where problem length >= N chars (default: 0)")
    args = parser.parse_args()

    base_dir = Path(args.base_dir) if args.base_dir else PROJECT_DIR
    eval_root = base_dir / "data" / "eval_files"
    out_path = Path(args.output) if args.output else base_dir / "high_quality_samples.log"
    ...
    for method in (["teler", "reasoning"] if args.method == "both" else [args.method]):
        ...
        for xlsx in xlsx_list:
            ...
            for _, row in df.iterrows():
                ...
                if avg is None or avg < args.min_avg:
                    continue
    ...
    if not all_rows:
        print(f"No rows with Average >= {args.min_avg} found", file=sys.stderr)
        return 1
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"High-quality samples (Average >= {args.min_avg}{len_desc}), n={len(samples)}\n")
        f.write("=" * 80 + "\n\n")
        ...
        for i, (method, row) in enumerate(samples, 1):
            ...
            f.write("-" * 80 + "\n")
            f.write(f"Sample {i} [{method}]  Dataset: {dataset}  Average: {avg_val}  (problem: {len(problem)} chars)\n")
            f.write("-" * 80 + "\n\n")
